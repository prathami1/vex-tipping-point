...
void anshSkills()
{
	...
	moveFourBar(0.9, false);
	moveclamp(1, true);
	moveFourBar(0.6, true);

	// prepping mogo and moving back, turning to prep for next stack
	movemogo(1, false);
	chassis->moveDistanceAsync(-1_ft);
	moveclamp(1, false);
	moveFourBar(2, false);
...
{
	// rules of thumb
	// turning: - is left, + is right
	// subsystems: up is true, down is false
	...
	// turn to clamp onto red mobile goal and face neutral mobile goal, driving to platform
	...
	// prepping to stack red mobile goal
	chassis->turnAngle(-80_deg);
	chassis->moveDistance(2_ft);
	moveFourBar(1, true);
	chassis->turnAngle(80_deg);
